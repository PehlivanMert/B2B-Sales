...
export default function CompanyDetailModal({ company, onClose, onUpdateStatus }) {
  ...
  const openedStatusRef = useRef(company?.status || 'lead');

  const { currentUser, userData } = useAuth();
  const { pushCrmUpdate } = useCompanyCrm();

  // Canlı veri varsa onu, yoksa prop'tan gelen veriyi kullan
  const currentStatus = liveData?.status ?? company?.status ?? 'lead';
  const liveLastUpdatedBy = liveData?.lastUpdatedBy ?? company?.lastUpdatedBy;
  const liveLastUpdatedAt = liveData?.lastUpdatedAt ?? company?.lastUpdatedAt;
  ...
  // Modal açıkken sadece bu acente'nin company_crm belgesi dinlenir.
  // Maliyet: 1 read/değişiklik — 18k koleksiyondan bağımsız.
  // Fayda: Başka kullanıcı aynı anda değiştirirse anında görünür.
  useEffect(() => {
    if (!company?.docId) return;
    const crmRef = doc(db, 'company_crm', company.docId);
    ...
    return () => unsub();
  }, [company?.docId, currentUser?.email]);

  // ── Notlar (company_crm/{docId}/notes) ───────────────────────────────────
  useEffect(() => {
    if (!company?.docId) return;
    const q = query(
      collection(db, 'company_crm', company.docId, 'notes'),
  ...
  const handleAddNote = async (e) => {
    e.preventDefault();
    if (!newNote.trim() || !company?.docId) return;
    setNoteError('');
    try {
      setIsSubmitting(true);
      await addDoc(collection(db, 'company_crm', company.docId, 'notes'), {
        text: newNote,
  ...
  const handleStatusChange = async (newStatus) => {
    if (!company?.docId || newStatus === currentStatus) return;
    setStatusError('');
    ...
    try {
      ...
      await pushCrmUpdate(company.docId, patch);
      
      if (onUpdateStatus) onUpdateStatus(company.docId, newStatus);
    } catch (error) {
  ...
  const handleDeleteNote = async (noteId) => {
    try {
      await deleteDoc(doc(db, 'company_crm', company.docId, 'notes', noteId));
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-full md:w-[500px] bg-white shadow-2xl z-50 flex flex-col transform transition-transform duration-300 ease-in-out border-l border-slate-200">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          <div>
            <h2 className="text-lg font-bold text-slate-800 line-clamp-1">{company.name}</h2>
            <p className="text-sm text-slate-500 mt-0.5">TÜRSAB No: {company.tax_no}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <section className="bg-slate-50 rounded-xl p-5 border border-slate-100">
            <h3 className="text-sm font-semibold text-slate-800 uppercase tracking-wider mb-4">Şirket Bilgileri</h3>
            <div className="space-y-4">
              <div className="flex gap-3 text-sm items-start">
                <MapPin className="w-5 h-5 text-slate-400 shrink-0 mt-0.5" />
                <div className="text-slate-600 flex-1">
                  <p className="font-medium text-slate-800">{company.city} / {company.district}</p>
                  <p className="mt-0.5">{company.address}</p>
                </div>
                {company.address && (
                  <a 
                    href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(company.address + ' ' + company.city + ' ' + company.district)}`}
                    target="_blank"
              ...
              <div className="flex items-center gap-3 text-sm justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-slate-700 font-medium">{company.phone || 'Belirtilmemiş'}</span>
                </div>
                {company.phone && (
                  <a 
              ...
              <div className="flex items-center gap-3 text-sm justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-slate-700 font-medium">{company.email || 'Belirtilmemiş'}</span>
                </div>
                {company.email && (
                  <a 
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-slate-700 font-medium text-xs bg-white px-2 py-1 rounded border border-slate-200">
                  Sektör: {company.sector || 'Bilinmiyor'}
                </span>
              </div>
            </div>
          </section>
