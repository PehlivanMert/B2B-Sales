...
export default function ImportModal({ onClose, agenciesMap }) {
    ...
    reader.onload = (event) => {
      try {
        ...
        json.forEach((row) => {
          ...
          // Find docId by tursab_no from existing agencies
          // agenciesMap is a mapping we need to pass: { '1234': 'docId' }
          const docId = agenciesMap[String(tursabNo)];
          if (docId) {
            validCount++;
        ...
        if (updates.length === 0) {
          setError('Dosyada geçerli "Belge No" ve "Durum" sütunu bulunamadı veya eşleşen acente yok.');
        } else {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setSuccess(`${preview.updates.length} acentenin durumu başarıyla güncellendi!`);
      setPreview(null);
      setFile(null);
        ...
        <div className="p-6">
          {!file && !success && (
            ...
            >
              ...
              <p className="text-xs text-slate-500 mt-1">Sütunlar: "Belge No" ve "Durum"</p>
            </div>
          )}
            ...
            <div className="mt-2 space-y-4">
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                ...
                <ul className="space-y-2 text-sm">
                  ...
                  <li className="flex justify-between">
                    <span className="text-slate-600">Bulunamayan Belge No:</span>
