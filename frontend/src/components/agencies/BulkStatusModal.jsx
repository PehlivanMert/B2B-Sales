...
export default function BulkStatusModal({ selectedAgencies, onClose }) {
  ...
  const handleUpdate = async () => {
    if (!selectedStatus || selectedAgencies.length === 0) return;
      ...
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl shadow-xl z-50 flex flex-col overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              <strong className="text-slate-700">{selectedAgencies.length}</strong> acente seçildi
            </p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6">
          <p className="text-sm text-slate-600 mb-4">
            Seçili {selectedAgencies.length} acentenin durumunu tek seferde değiştirebilirsiniz. Bu işlem geri alınamaz.
          </p>
