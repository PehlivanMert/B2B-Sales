...
import {
  getAllCrmEntries, getCrmEntryCount, putCrmEntries,
  patchCrmEntry, getLastSyncedAt, setLastSyncedAt, clearCrmDb,
...
/**
 * Tüm agency_crm koleksiyonunu 500'lük sayfalar halinde çeker,
 * IndexedDB'ye yazar ve React state'i günceller.
 */
async function fullSync(setCrmData, setSyncInfo) {
  const col = collection(db, 'agency_crm');
  let lastDoc = null;
  let allData = {};
  let totalFetched = 0;
  ...
  try {
    ...
    console.error('CrmContext fullSync hatası:', err);
  }
}
...
async function deltaSync(lastSyncedAt, setCrmData, setSyncInfo) {
  try {
    ...
    if (err.code === 'failed-precondition' || err.code === 'unimplemented') {
      console.warn('CrmContext: Delta sync index yok, full sync çalışıyor. Firestore konsolundan index oluşturun:', err.message);
    } else {
      console.error('CrmContext deltaSync hatası, full sync\'e düşülüyor:', err);
    }
...
export function CrmProvider({ children }) {
    ...
    function handleVisibility() {
      if (document.hidden) {
        ...
        if (away >= VISIBILITY_THRESHOLD_MS) {
          getLastSyncedAt().then(lastSyncedAt => {
  ...
  const updateCrmEntry = useCallback((docId, patch) => {
    ...
    patchCrmEntry(docId, patch).catch(console.error);
  }, []);
  ...
  const batchUpdateCrmEntries = useCallback((entries) => {
    ...
    Promise.all(entries.map(({ docId, patch }) => patchCrmEntry(docId, patch)))
      .catch(console.error);
  }, []);
  ...
  const flushPendingWrites = async () => {
    ...
    try {
      ...
      for (const chunk of chunks) {
        ...
        chunk.forEach(w => {
          b.set(doc(db, 'agency_crm', w.docId), w.patch, { merge: true });
        });
        await b.commit();
  ...
  const pushCrmUpdate = async (docId, patch) => {
    ...
    if (navigator.onLine) {
      try {
        await setDoc(doc(db, 'agency_crm', docId), patch, { merge: true });
      } catch (err) {
        // Hata alırsak kuyruğa at
        await enqueuePendingWrite(docId, patch);
      ...
      try {
        ...
        for (const chunk of chunks) {
          ...
          chunk.forEach(({ docId, patch }) => {
            batch.set(doc(db, 'agency_crm', docId), patch, { merge: true });
          });
          await batch.commit();
        }
      } catch (err) {
